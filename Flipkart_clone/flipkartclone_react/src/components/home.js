...
const Home=()=>{
  ...
  let getData=async()=>{
    ...
    let result=await get(url,option);
    if(result)
        ...
        <div className="container">
            <div className="d-flex justify-content-between catemobile gap-3">
                { categorylist.map((category)=>{
                    return(
                        <div  onClick={()=>navigate(`/products?c_id=${category.c_id}&cat_name=${category.category}`)} className="items text-center" key={category.c_id}>
                        <div className="catimg">
                            <img width="64px" height="64px" src={category.image} alt=""/>
                        </div>
